...
if ($days != "NULL") {
	...
	//query db
	$query = "SELECT jobs.job_id, jobs.client_id, jobs.job_number, jobs.job_title, jobs.employee, jobs.order_date, jobs.due_date, jobs.total_hours, jobs.external_hours, jobs.billable_hours, jobs.project_manager, clients.client_name FROM jobs, clients WHERE jobs.client_id = clients.client_id AND jobs.status = 'open' AND jobs.est_completion = '$timeperiod' AND jobs.employee LIKE '%$myid%' ORDER BY client_name, job_id";
} else {
	$query = "SELECT jobs.job_id, jobs.client_id, jobs.job_number, jobs.job_title, jobs.employee, jobs.order_date, jobs.due_date, jobs.total_hours, jobs.external_hours, jobs.billable_hours, jobs.project_manager, clients.client_name FROM jobs, clients WHERE jobs.client_id = clients.client_id AND jobs.status = 'open' AND jobs.employee LIKE '%$myid%' ORDER BY client_name, job_id";
}
